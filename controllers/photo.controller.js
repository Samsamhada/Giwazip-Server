            ...
            .catch((err) => {
                ...
                console.log(
                    `[${moment().format("YYYY-MM-DD HH:mm:ss.SSS")}] ` +
                        ...
                        " " +
                        err.message ||
                        "Photo 테이블을 조회하는 중에 문제가 발생했습니다. (IP: " +
                            (req.header("X-FORWARDED-FOR") ||
                                req.socket.remoteAddress) +
                            ")"
                );
...
exports.findOne = (req, res) => {
    ...
    if (req.header("API-Key") == apiKey) {
        Photo.findByPk(id)
            .then((data) => {
                if (data) {
                    ...
                    console.log(
                        `[${moment().format("YYYY-MM-DD HH:mm:ss.SSS")}] ` +
                            chalk.bgGreen("Success:") +
                            ` Photo 테이블의 id=${id}번 데이터를 성공적으로 조회했습니다. (IP: ` +
                            (req.header("X-FORWARDED-FOR") ||
                                req.socket.remoteAddress) +
                            ")"
                    );
                    ...
                    res.status(400).send({
                        message: `Photo 테이블에서 id=${id}번 데이터를 찾을 수 없습니다.`,
                    });
                    console.log(
                        `[${moment().format("YYYY-MM-DD HH:mm:ss.SSS")}] ` +
                            chalk.bgRed("Error:") +
                            ` Photo 테이블에서 id=${id}번 데이터를 찾을 수 없습니다.` +
                            " (IP: " +
                            (req.header("X-FORWARDED-FOR") ||
                                req.socket.remoteAddress) +
                            ")"
            ...
            .catch((err) => {
                res.status(500).send({
                    message: `Photo 테이블의 id=${id}번 데이터를 조회하는 중에 문제가 발생했습니다.`,
                });
                console.log(
                    `[${moment().format("YYYY-MM-DD HH:mm:ss.SSS")}] ` +
                        chalk.bgRed("Error:") +
                        ` Photo 테이블의 id=${id}번 데이터를 조회하는 중에 문제가 발생했습니다.`
                );
            });
    } else {
        res.status(401).send({ message: "Connection Fail" });
